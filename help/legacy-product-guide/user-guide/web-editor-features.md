...
- **发布配置文件**：这包含可用于发布&#x200B;**知识库**&#x200B;输出的发布配置文件。 您可以为目标知识库创建新的用户档案。 例如，Salesforce或ServiceNow。
  ...
  这些资源可用于创建Salesforce发布配置文件。
   ...
   - 要创建Salesforce发布配置文件，请从&#x200B;**服务器类型**&#x200B;下拉列表中选择&#x200B;**Salesforce**&#x200B;知识库。输入配置文件名称。在&#x200B;**站点URL**&#x200B;中，输入用于发布输出的使用者站点，然后添加由Salesforce使用者站点提供的&#x200B;**使用者密钥**&#x200B;和&#x200B;**使用者密钥**。然后，**验证**&#x200B;并&#x200B;**保存**&#x200B;新创建的配置文件。
     在编辑器设置中![salesforce发布配置文件](./images/salesforce-publish-profile.png){width="550"}
...
- **验证**：此选项卡包含用于在Web编辑器中配置架构验证的选项。 您可以启用以下功能：
   ...
   - **在保存文件之前运行验证检查**：选择此项可在任何保存操作之前使用选定的架构文件运行架构验证。 您可以通过单击+图标来添加Schematron文件。 将列出选定的架构文件。
     ...
     在编辑器设置中验证![](./images/editor-setting-validation.png){width="550"}
...
- **元数据**：您可以控制主题的版本元数据及其值显示在&#x200B;**版本历史记录**&#x200B;对话框中。  在元数据路径中，指定要从中选取元数据的节点的位置。 您还可以将元数据的自定义名称定义为标签。 默认属性为“标题”、“文档状态”和“标记”。
  ...
  编辑器设置中的![元数据选项卡](images/editor-setting-metadata.png){width="550"}
...
- **常规**：“常规”选项卡允许您配置以下设置：
  ...
  ![用户首选项的“常规”选项卡](images/user_preference_editor.PNG){width="550"}
...
- **外观**：选择Web编辑器应用程序的主题和内容编辑区域的源视图。
  ...
  用户首选项的![外观选项卡](images/user_preference_editor_appearance.png){width="550"}
...
![插入元素](images/insert-element.png){width="300"}

*输入&#39;t&#39;以搜索所有以&#39;t&#39;开头的有效元素。*

如果您在块元素（如`note`）中工作，则使用“插入元素”图标在`note`元素之后插入新元素。 在以下屏幕快照中，注释元素已插入p \(paragraph\)元素中：

![在块元素中插入元素](images/note-in-para-insert-element_cs.png){width="800"}
...
1. 单击&#x200B;**版本历史记录**。
   ...
   ![版本历史记录对话框](images/version-history-dialog-web-editor.png){width="550"}
   *预览不同版本主题中的更改。*
...
1. 您还可以在&#x200B;**版本历史记录**&#x200B;对话框中查看以下信息：
   ...
   **元数据**&#x200B;选项卡：新添加的元数据为绿色字体，已删除的元数据为红色字体。
   ![版本](images/metadata-version-diff.png){width="550"}的元数据差异
   *在版本历史记录中比较不同版本的元数据。*
...
![在存储库视图中搜索文件](images/repository-filter-search.png){width="300"}
...
![高级筛选器对话框](images/repository-filter-search-advanced.png) {width="800"}
...
存储库视图中文件的![选项菜单](images/options-menu-repo-view-file-level.png){width="550"}
...
映射视图中的![键](images/view-key-title-map-view.png){width="300"}
...
- 单击“大纲视图”中的某个元素，会在“创作”或“Source”视图中选择该元素的内容。“大纲”视图与“创作”视图和“Source”视图保持同步。 如果在任何视图中进行了任何更改，则可在“大纲”视图中看到这些更改。 例如，如果在“创作”视图中添加段落或更新元素，则它将显示在“大纲”视图中。
...
以嵌套层次结构组织的![条件](images/conditions-nested-hierarchy.png){width="300"}
...
1. 在左侧面板上选择“审阅![](images/active-review-tasklist-icon.svg)”。 将打开&#x200B;**审阅**&#x200B;面板。  系统将显示您所属的审阅项目中的所有审阅项目和活动审阅任务。
...
1. 要编辑属性，请将鼠标悬停在它上并选择&#x200B;**编辑** ![编辑图标](images/edit_pencil_icon.svg)。
   ![编辑属性](images/edit-attributes-content-properties.png){width="300"}
