...
- [从“地图”控制台中](#condition-presets-from-the-map-console)
- [从地图仪表板](#condition-presets-from-the-map-dashboard)
...
1. 导航到左侧的&#x200B;**条件预设**。 将显示为DITA映射定义的条件预设列表。
...
1. 执行以下任一操作以添加属性：
   ...
   - **全选**&#x200B;以将所有属性及其值添加到条件预设。 默认情况下，属性的操作为`Include`。

1. 选择&#x200B;**添加**。 您可以重复此步骤以添加更多属性。 您添加的属性从中央移至右侧面板。
1. 从顶部的操作栏中选择&#x200B;**删除**&#x200B;以删除右侧面板中的选定属性。
1. （可选）如果需要，您可以覆盖应用于属性的操作。

   执行以下操作之一：
对于任何属性，请从“操作”下拉列表或工具栏中选择以下操作之一。
   ...
   从右侧面板中选择多个属性行，然后从顶部的操作栏中选择一个操作。 例如，您可以为所选属性选择&#x200B;**排除**&#x200B;操作。
1. 选择&#x200B;**保存**&#x200B;以保存条件预设。
...
创建条件预设后，该预设会显示在&#x200B;**输出预设**&#x200B;的&#x200B;**条件预设**&#x200B;下拉列表下。 详细了解如何[发布PDF输出](../web-editor/native-pdf-web-editor.md)。
...
1. 执行以下任一操作以添加属性：
   - 选择&#x200B;**将**&#x200B;添加到条件预设的一个属性。 您可以重复此步骤以添加更多属性。
   - 选择&#x200B;**全部添加**&#x200B;以将所有属性添加到条件预设。
1. \（可选\）如果需要，可以覆盖在步骤4中应用于属性的默认操作。 执行下列操作之一：
   - 选择多个属性，从&#x200B;**中选择一个操作将所选条件的操作设置为**，然后选择&#x200B;**应用**。
   - 从&#x200B;**操作**&#x200B;下拉列表中选择属性的操作。
1. 选择&#x200B;**保存**。
