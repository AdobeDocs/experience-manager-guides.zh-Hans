...
- **编辑器**：允许您发送单个主题或DITA映射以供审阅。 请注意，创建审阅任务的工作流在编辑器和Assets UI中是通用的。 只有启动审阅工作流的方法不同。 有关从编辑器启动审阅工作流的信息，请查看编辑器中的[创建审阅任务](web-editor-left-panel.md#review)功能。
...
1. 在&#x200B;**分配给**&#x200B;下拉字段中，选择要向其发送审阅主题的审阅人。 键入内容时，您还可以在所选项目中按成员名称或角色名称搜索。

   仅当您在&#x200B;*项目*&#x200B;字段中选择活动项目后，*分配给*&#x200B;字段才会启用。 将根据所选项目获取成员或角色的列表。 如果未选择任何项目，则&#x200B;*分配给*&#x200B;字段将保持禁用状态。

   您可以将审阅任务分配给项目的单个用户或用户组。 请注意，仅当您是项目的管理员组的一部分时，才能将审核任务分配给个人用户，否则将仅在&#x200B;*分配给*&#x200B;字段中查看用户组。

   >[!NOTE]
   >
   > 审核工作流特定于项目。 创建项目时，可以将团队成员添加到项目中，并将他们分配给组。 因此，当您在此处选择项目时，您有权选择属于该项目的成员。 有关项目的详细信息，请查看[创建DITA项目](authoring-create-dita-project.md#)。
...
1. 启用&#x200B;**自动提醒**&#x200B;为审阅者配置计划的提醒通知（和电子邮件提醒）。 这既可以在审核任务的到期日期之前完成，也可以在审核任务过期之后完成。

   >[!NOTE]
   >
   >自动提醒切换保持禁用状态，直到您为审核任务选择截止日期为止。

   - **到期日期提醒之前**：您最多可以在到期日期之前60天配置多个提醒（最多3个）以提前通知审阅人。 提醒1可以设置为从今天到到期日期的剩余天数，并且提醒值必须遵循降序顺序，表示到期日期之前的天数。

   - **到期日期提醒之后**：您还可以配置提醒（最多3个），使其在到期日期之后发送，以通知审阅者任务已过期。 过期提醒最多可在到期日期后30天内进行配置。 提醒值必须跟在升序之后，表示到期日期后的天数。

   根据配置的时间表，系统在计划的提醒日期向审阅人发送AEM通知和电子邮件提醒。
...
1. 选择&#x200B;**下一步**。
   ...
   >[!NOTE]
   >
   > 只有在您为所有必填字段提供了详细信息（包括&#x200B;*标题*、*项目*、*分配给*&#x200B;和&#x200B;*截止日期*）之后，才会启用&#x200B;**下一步**&#x200B;按钮。
...
作为审阅任务的作者或发起者，您可以向正在进行的审阅任务添加新主题（如果先前未发送这些主题以供审阅），或从正在进行的审阅任务中删除主题，而不会影响审阅工作流。

执行以下步骤可在正在进行的审阅任务中添加或删除主题：

1. 在“审阅”面板中，导航到要修改的审阅任务，然后从上下文菜单中选择&#x200B;**打开任务详细信息**。

   显示&#x200B;**任务详细信息**&#x200B;页面。

1. 选择&#x200B;**CONTENT**&#x200B;选项卡以查看当前包含在审核任务中的主题。
1. 查看主题列表并根据需要修改所选内容。
   ...
   >[!NOTE]
   >
   > 必须至少保持选择一个主题才能更新正在进行的审核任务。

   ![](images/modify-review-topics.png)

1. 修改主题列表后，相应的更新将自动传播到“任务详细信息”页面的&#x200B;**审阅者**&#x200B;选项卡。

   ![](images/modify-reviewers.png)

   查看“查看者”选项卡中更新的主题列表。 对于添加的任何新主题，您需要分配审阅人，除非为您的审阅任务启用了&#x200B;**允许被分派人审阅任何主题**&#x200B;设置。
1. 选择&#x200B;**更新**。

更新审阅任务后，审阅人会根据其审阅状态接收通知（AEM和电子邮件）：

- **进行中**：正在处理任务的审阅人将收到任务已更新的通知。
- **已完成**：已完成审阅的审阅人会收到请求重新审阅的通知。
