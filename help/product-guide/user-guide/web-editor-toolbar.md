...
通过菜单下拉菜单可访问各种编辑操作，如下所示：

>[!BEGINTABS]

>[!TAB 新编辑器]

编辑器工具栏的![菜单下拉列表](images/editor-toolbar-menu-dropdown-new-editor-2-0.png){width="350"}


>[!TAB 旧编辑器]

编辑器工具栏的![菜单下拉列表](images/editor-toolbar-menu-dropdown-new.png){width="350"}
...
>[!NOTE]
>
> 在新编辑器中，**显示菜单**&#x200B;在菜单下拉菜单中不再可用。 要访问“显示”菜单下的各种可用选项，请导航到[编辑器设置](./config-editor-settings.md)。 “显示”菜单在旧编辑器的“菜单”下拉菜单中继续可用。
...
![在块元素中插入元素](images/note-in-para-insert-element_cs-editor-2-0.png)

>[!TAB 旧编辑器]

![在块元素中插入元素](images/note-in-para-insert-element_cs.png)
...
在新编辑器中，当出现水平光标时，可在现有元素之间插入新元素。 此光标指示光标位于块元素的外部，并且可以在该位置添加新元素。

![](images/Block-cursor-editor-2-0.png){width="300"}

例如，在DITA主题中，当水平光标出现在`short description`和`body`之间时，您可以插入`prolog`元素。 然后，您可以使用`prolog`添加元数据，如`copyright`、`author`和其他详细信息。
...
在当前或下一个有效位置插入简单表或表。 选择“表”图标以打开&#x200B;**插入简单表**&#x200B;对话框。
...
>[!NOTE]
>
> 您还可以选择切换到新编辑器，以改善表编辑体验。 有关新编辑器中可用的表创作功能的更多详细信息，请查看新编辑器中的[处理表](./web-editor-other-features.md#work-with-tables-in-the-new-editor)。
...
**Source路径**&#x200B;字段显示所选引用的相对路径。 使用&#x200B;**复制路径**&#x200B;按钮复制绝对路径。

**Source UUID**&#x200B;字段显示所选引用的UUID。 使用&#x200B;**复制UUID**&#x200B;按钮复制UUID。
...
在新编辑器中，您还可以通过在创作模式下拖动图像的句柄来调整图像大小：

- **角手柄**：拖动角手柄可调整图像大小，同时保持其长宽比。

  ![](images/resize-corner-handles.png)

- **中间手柄**：按&#x200B;**Shift**&#x200B;显示其他中间手柄。 拖动这些参数可调整图像大小而不保持纵横比，从而使您可以单独增加或减少高度或宽度。

  ![](images/resize-middle-handles.png)

拖动时，**内容属性**&#x200B;面板中的“高度”和“宽度”值会自动更新以反映新维度。 此大小调整也反映在&#x200B;**并排**&#x200B;模式和&#x200B;**预览**&#x200B;模式中，因此您可以确认调整后的图像在发布前的显示方式。
...
>[!NOTE]
>
>您还可以在图像中定义可单击区域\（图像映射\）。 有关更多详细信息，请在[左侧面板](web-editor-left-panel.md)部分中查看&#x200B;**插入/编辑图像映射**&#x200B;功能说明。
...
>[!IMPORTANT]
>
> 即使被引用内容的相对路径显示在&#x200B;**链接路径**&#x200B;属性中，链接在内部仍使用被引用内容的UUID创建。
...
版本信息显示主题或映射的当前版本。 版本号旁边显示一个星号`(*)`，表示更改已保存在文档中，但未进行版本控制。
