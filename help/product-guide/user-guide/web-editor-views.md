...
Adobe Experience Manager中的编辑器界面支持以四种不同的模式或视图查看主题：
...
这是典型的&#x200B;**您所看到的内容是您获取编辑器的** \(WYSISYG\)视图。 您可以像在任何常规富文本编辑器中一样编辑主题。 在“作者”视图中，您可以选择保存文档的修订版本、查找和替换内容、插入元素、插入超链接、插入内容引用等。
...
有关更多详细信息，请在[选项卡栏](web-editor-tab-bar.md)部分中查看&#x200B;**查找和替换**&#x200B;功能说明。
...
通过并排视图，可在同一屏幕上同时查看和工作创作视图和Source视图。 WYSIWYG Author视图和底层XML Source视图彼此相邻显示，支持并行内容和结构编辑，而无需切换视图。 两个视图保持实时同步，确保创作视图中的光标位置和选择反映在Source视图中的相应位置，在创作结构化内容时提供更好的准确性和控制。
...
PDF是在文档开发周期的每一个可能阶段使用的最常见输出格式之一。 Experience Manager Guides使您可以灵活地生成单个主题的PDF或整个映射文件。 导出为PDF功能允许作者、发布者或管理员轻松生成单个主题的PDF输出。 它使用保存在文件夹级别配置文件中的DITA-OT配置来生成PDF。
...
1. 在预览模式下打开主题。 确保该主题是映射文件的一部分。

1. 从顶部选择&#x200B;**下载为PDF**&#x200B;选项。

   ![](images/download-as-pdf-preview.png)图标。
