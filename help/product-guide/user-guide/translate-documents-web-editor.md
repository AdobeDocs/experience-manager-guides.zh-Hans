...
1. 作为用户，您可以查看为文件夹配置文件配置的语言组。 语言组显示语言文件夹及其语言代码。 例如，名为G1的语言组包含意大利语\(it\)、德语\(de\)、法语\(fr\)和英语\(en\)语言文件夹。
   ...
   ![翻译面板](images/translation-languages.png){width="300"}
...
1. 选择&#x200B;**应用**。 此时将显示一个列表，其中包含主题和相关资源的详细信息。
   ...
   >[!NOTE]
   >
   > 在使用DITAVAL和Markdown文件引用翻译映射时，以及当翻译基于工作副本时，如果源语言文件夹中存在图像和其他链接资产等引用，则会包含这些引用。 这些引用出现在翻译操控板的引用列表中，使您能够显式选择它们进行翻译。 在翻译过程中，引用的资产会复制到目标语言文件夹中，并按照标准图像翻译行为一致地处理。
...
将使用所选主题版本创建新翻译项目。 此时，会显示一个弹出消息，确认已创建翻译项目。 一旦翻译项目中的所有目标语言副本均可用，您即会在收件箱中收到通知。 一旦目标语言副本在翻译项目中可用，您就可以开始翻译作业。 有关详细信息，请[启动翻译作业](translation-first-time.md#id225IK030OE8)。
...
要在目标副本中添加源版本标签，系统管理员必须在&#x200B;**Workspace设置**&#x200B;中的&#x200B;**翻译**&#x200B;选项卡下启用&#x200B;**将源版本标签传播到目标版本**&#x200B;选项（对于&#x200B;**内部部署5.2**&#x200B;之前的安装程序，显示为&#x200B;**设置**）。
...
1. 选择&#x200B;**强制同步**。 它会将选定的“不同步”资产的状态设置为“同步”。
