...
1. 要更新元数据，请选择要更新的文件。 通过选中显示在&#x200B;**Title**&#x200B;旁边的复选框，可以选择出现在元数据面板中的文件。

   如果要选择所有记录，请使用标题栏上方的&#x200B;**全选**&#x200B;选项卡。 由于记录会在滚动时逐渐加载，因此在使用&#x200B;**全选**&#x200B;之前，请确保滚动到列表的末尾。 这可确保评估所有文件的编辑权限和锁定状态，而不仅仅是当前可见的记录。

   >[!NOTE]
   >
   > 如果某个文件被其他用户锁定，则无法选择该文件来更新元数据。 锁定图标也会出现在锁定文件的标题附近。 您可以将鼠标悬停在图标上以查看用户的名称。 您可以将&#x200B;**管理**&#x200B;按钮配置为在所选文件处于只读模式时将其禁用。 有关详细信息，请在“元数据”面板](../guides-ui-extensions/examples.md)上查看[管理按钮。
   >
   > <br>此限制仅适用于DITA和Markdown文件。 对于非DITA资产（例如图像和多媒体），即使处于只读模式，元数据属性也保持可编辑状态。
...
1. 要添加自定义元数据列，请从元数据页面右上角的三个圆点中选择&#x200B;**Workspace设置**(显示为&#x200B;**内部部署（版本5.2**&#x200B;之前） **设置**)。
...
1. 您可以根据&#x200B;**文件类型**&#x200B;和&#x200B;**链接类型**&#x200B;筛选断开的链接。 系统会根据您在下拉列表中的选择，显示断开的链接列表。 例如，您可以选择仅显示DITA映射中的内容引用，而文件仅显示其中使用的已断开的内容引用。
   ...
   DITA主题、DITA映射、文件引用、键引用、内容引用、内容键引用、图像引用、多媒体文件引用和多媒体键引用是&#x200B;**链接类型**&#x200B;下拉菜单的可用值，主题、映射、文档、图像、视频、音频和其他是&#x200B;**文件类型**&#x200B;下拉菜单的可用值。
1. 您还可以使用以下筛选选项来选择在列表中显示以下列：

   - **断开的链接**（默认选中）断开的链接的路径在DITA映射中指定。

   - **链接类型**（默认选中）链接的类型。 可用的选项有“主题”、“映射”、“文件引用”、“键引用”、“内容引用”、“内容键引用”、“图像引用”、“多媒体文件引用”和“多媒体键引用”。

   - **在**&#x200B;中使用（默认选中）已使用断开链接的引用。 您可以选择引用以在创作模式下查看它。

   - **文件类型**（默认选中）引用的类型 — 主题、地图、文档、图像、视频、音频和其他。
