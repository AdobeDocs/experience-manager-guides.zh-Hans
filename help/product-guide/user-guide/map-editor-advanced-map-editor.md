...
- [源](#source-view)
- [预览](#preview)
- [并排视图](#side-by-side-view-available-in-new-editor-only)
...
>[!NOTE]
>
>您还可以拖放引用以在地图中移动它们。 在拖放引用后如何处理引用取决于您使用的编辑器。 在编辑器2.0中，将引用（如主题、子映射或键定义）拖放到映射中的现有引用上，会将其添加为子元素，而不是替换现有引用。 在旧编辑器中，相同的操作会更新引用的URL，而不是将其添加为子元素。
...
**作者**&#x200B;视图允许您在编辑器中编辑DITA映射。 这会显示映射编辑器的WYSIWYG视图。
...
**菜单**&#x200B;下拉列表中的可用选项与[编辑器工具栏](./web-editor-toolbar.md#menu-dropdown)中的可用选项相同。

此外，您还可以从“作者”视图中查看以下图标并执行相关任务：

**此项前插入/此项后插入** - ![](images/insert_element_before_icon.svg) / ![](images/insert_element_after_icon.svg)

显示&#x200B;**在**&#x200B;之前插入元素或&#x200B;**在**&#x200B;之后插入元素。 选择要插入到映射中的元素。 根据操作，新元素会插入到映射中当前元素之前或之后。
...
>[!NOTE]
>
> 在新编辑器中，编辑器设置下提供了用于添加或删除&#x200B;**标记**&#x200B;的选项。 有关详细信息，请查看[编辑器设置](./config-editor-settings.md)。
...
通过新编辑器中的并排视图，您可以查看彼此相邻的“创作”视图和“Source”视图。 这样可以更便于了解如何构建映射，并确保所做的任何更改都与预期的结构保持一致。

例如，如果主题未正确显示在输出中，则可以使用作者视图检查其在地图中的位置，同时查看Source视图以查看一个或多个主题引用。
...
1. 导航到Explorer并打开要编辑的映射文件。

   映射文件在“映射”视图中打开。

   >[!NOTE]
   >
   > 您还可以使用Assets UI打开映射文件。 导航到包含要编辑的主题的DITA映射文件，然后在主工具栏中选择&#x200B;**编辑主题**&#x200B;以启动编辑器。

1. 选择任意主题以在编辑器中打开该主题进行编辑。
...
右侧面板在映射编辑器的布局视图中显示内容属性和映射属性。

**内容属性**

“内容”属性面板包含有关图表中当前选定主题的信息，包括其类型、链接路径、链接UUID和属性。

有关详细信息，请在编辑器中查看[内容属性](web-editor-features.md#right-panel)。
...
请参阅下面的选项卡，比较属性在新编辑器和旧编辑器的创作视图中的显示方式。

>[!BEGINTABS]

>[!TAB 新编辑器]

![作者视图](images/author-inline-attributes-editor-2-0.png){width="650"}

>[!TAB 旧编辑器]

![作者视图](images/author-inline-attributes.png){width="650"}
