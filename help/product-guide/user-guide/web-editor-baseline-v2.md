...
| 区域 | 更改（描述） |
|------|-------------|
| **引用分辨率** | 直接映射引用被分类为&#x200B;**DIRECT**。 跳过了无效的引用，继续排除来自`reltable`的引用。 已编辑的基线和新基线均支持此功能，但仅已迁移的基线则不支持。 |
| **自动挑选** | 在解决直接引用之前立即评估版本选择，以确保精确的版本分辨率。 已编辑的基线和新基线均支持此功能，但仅已迁移的基线则不支持。 |
...
1. 在对话框中提供以下详细信息：
   ...
   1. **功能类型**：从下拉列表中选择&#x200B;**基线**。
   1. **选择文件夹和文件**：导航并选择一个或多个要处理的文件夹和文件。 只能选择文件夹进行基线迁移。

      >[!NOTE]
      >
      > 选择包含所有指南内容和映射文件的文件夹。 如果映射文件单独存储，请选择映射文件所在的目录。
