...
>[!NOTE]
>
> 作为管理员，您还可以选择在编辑器中按文件名查看文件列表。 在&#x200B;**用户首选项**&#x200B;的&#x200B;**编辑器文件显示配置**&#x200B;节中选择&#x200B;**文件名**&#x200B;选项。
...
>[!NOTE]
>
> 作为管理员，您还可以选择在编辑器中按文件名查看文件列表。 在&#x200B;**用户首选项**&#x200B;的&#x200B;**编辑器文件显示配置**&#x200B;节下选择&#x200B;**文件名**&#x200B;选项。
...
![高级筛选器对话框](images/repository-filter-search-advanced.png) {width="650"}
...
1. 选择&#x200B;**创建**。 该主题在指定的路径中创建。 此外，该主题将在编辑器中打开以进行编辑。
...
- **上传资源**：将文件从本地系统上传到Adobe Experience Manager存储库中的选定文件夹。 您还可以将文件从本地系统拖放到当前工作主题上。 如果要将本地系统中的图像插入主题，这非常有用。
  ...
  <summary>处理文件名中的无效字符</summary>

  如果上传资源的文件名包含无效字符(如* / ： [\] | # % { } ？)，可能会出现以下情况：

  - **部分上传**：指示要上传的一个或多个资源的文件名中包含无效字符。

    ![](images/partial-asset-upload.png)

  - **上载失败** ：指示所有要上载的资产在其文件名中包含无效字符。

    ![](images/upload-asset-failed.png)

  要解决这些冲突，请从资产的文件名中删除所有无效字符，然后重新将其上传到存储库。
...
- **属性**：使用此项可打开所选文件的属性页。
  ...
  任何在此页面上添加、删除或修改元数据属性（无论是默认还是自定义）都将触发文档版本上的[工作副本指示器](./web-editor-edit-topics.md#working-copy-indicator)。

  您还可以从Assets UI访问“属性”页面，方法是选择一个文件，然后选择工具栏中的“属性”图标。
...
但是，它引入了增强的搜索和筛选体验，旨在提高可用性和效率。

有关文件和文件夹上下文菜单的详细信息，请查看[选项菜单](#options-menu)。

有关搜索体验的详细信息，请查看[搜索面板](./search-panel-explorer.md)。
...
- **在资源管理器中定位**：显示映射文件在资源管理器中的位置\（或DAM\）。
...
- **运行运行状况检查**：启用此选项以在所选映射上运行运行状况检查。 有关此功能的更多详细信息，请在映射编辑器中查看[其他功能](./map-editor-other-features.md#run-health-check-on-a-map)。
  >[!NOTE]
  >
...
- **在资源管理器中定位**：显示所选文件在资源管理器\（或DAM\）中的位置。
- **全部展开**：展开映射文件中的所有主题。
...
- 选择可重用内容旁边的&#x200B;**+**&#x200B;图标以打开&#x200B;**选择文件**&#x200B;对话框。
- 选择要添加的文件，然后选择&#x200B;**选择**。 您还可以使用过滤器搜索选项搜索特定文件。 有关更多详细信息，请查看编辑器中的[其他功能](./web-editor-other-features.md)。
...
如果管理员为属性创建了配置文件，则您将获得这些属性及其配置的值。 您还可以在&#x200B;**Workspace设置**&#x200B;的&#x200B;**显示属性**&#x200B;选项卡下分配管理员配置的显示属性（显示为&#x200B;**内部部署设置的**&#x200B;设置&#x200B;**，此设置早于版本5.2**）。 为元素定义的属性将显示在“布局”和“大纲”视图中。
...
若要从“条件”面板查看主题方案，系统管理员必须在&#x200B;**Workspace设置**&#x200B;的“常规”选项卡下选择&#x200B;**在“条件”面板中显示主题方案**&#x200B;选项（对于&#x200B;**内部部署，显示为**&#x200B;设置&#x200B;**，此设置早于5.2**&#x200B;版本）。 有关详细信息，请查看[选项卡栏](./web-editor-tab-bar.md)。
...
1. 选择代码片段旁边的&#x200B;**+**&#x200B;图标以打开&#x200B;**新建代码片段**&#x200B;对话框。

   ![](images/snippet-new-dialog.png){width="300"}

   在“新建代码片段”对话框中，提供显示在“代码片段”面板中的标题、说明，为您的内容选择格式（DITA或HTML），并提供要创建的代码片段内容的代码。 选择&#x200B;**创建**&#x200B;以保存并创建代码片段。

2. 在内容编辑区域中，右键单击要用作代码片段的元素的痕迹导航，然后从上下文菜单中选择&#x200B;**创建代码片段**。 此时将显示“新建代码片段”对话框，并在&#x200B;**Content**&#x200B;字段中填充了选定元素的XML代码。 输入该代码片段的&#x200B;**Title**&#x200B;和&#x200B;**Description**，然后选择&#x200B;**Create**&#x200B;保存该代码片段。

3. 在内容编辑区域中，右键单击要用作代码片段的内容的任意位置，然后从上下文菜单中选择&#x200B;**创建代码片段**。 此时将显示“新建代码片段”对话框，并在&#x200B;**Content**&#x200B;字段中填充了选定元素的XML代码。 输入该代码片段的&#x200B;**Title**&#x200B;和&#x200B;**Description**，然后选择&#x200B;**Create**&#x200B;保存该代码片段。
...
- 从“代码片段”面板中选择一个代码片段，并将其拖放到主题中的所需位置。 您还可以使用“代码片段”面板顶部的过滤器选项来优化您的视图：

  - **显示所有代码片段**：列出所有可用的代码片段，包括DITA和HTML格式。
  - **仅显示适用的代码片段**：筛选列表以仅显示与您当前主题或上下文相关的代码片段。 例如，如果您正在处理DITA主题，HTML代码片段将从列表中排除，以确保上下文准确性。
...
>[!NOTE]
>
> 仅在旧编辑器中支持引用，当前在新编辑器中不可用。 本文中的信息仅适用于使用旧编辑器的情况。
...
查找和替换功能可在两种不同的模式下使用，具体取决于您的要求：

- **使用Source模式：**&#x200B;搜索不仅扫描可见内容，还扫描搜索字符串的基础源内容（XML结构，包括元素、标记和属性值）。 此模式可确保跨整个内容进行全面的搜索。 若要使用此功能，必须启用“查找和替换”面板中可用的&#x200B;**使用源模式**&#x200B;选项。

  >[!NOTE]
  >
  > **使用源模式**&#x200B;功能在2026.01.0版的Experience Manager Guides as a Cloud Service中可用，并适用于5.2版的内部部署设置。 要使用此功能，您必须首先根据使用的设置为内容编制索引。
  > - **对于云服务**：必须执行自定义索引部署。 有关详细信息，请查看[正在部署自定义索引](/help/product-guide/cs-install-guide/custom-indexing.md)。 完成后，请联系您的客户成功团队以启用此功能。
  > - **对于内部部署**：在使用该功能之前，需要重新索引现有内容。 有关详细信息，请查看[重新索引内容](/help/product-guide/install-guide/custom-indexing-prem.md)。 完成后，请联系您的客户成功团队以启用此功能。
...
<details>
    <summary> 执行全局搜索并替换为Source模式</summary>

要执行全局搜索并替换为Source模式，请执行以下步骤：

1. 打开全局&#x200B;**查找和替换**&#x200B;面板。
1. 启用&#x200B;**使用源模式**&#x200B;选项。
1. 选择&#x200B;**路径**&#x200B;下拉列表并选择以下选项之一来执行搜索。

   - **路径**：要在所选路径上搜索
   - **映射**：从存储库或收藏集中搜索某些特定映射
   - **当前映射**：要在当前打开的映射中搜索

   ![](images/path-dropdown-source-mode.png){width="350"}


1. 在&#x200B;**查找**&#x200B;字段中输入搜索字符串。 要缩小结果范围，请选择“查找”字段旁边的&#x200B;**筛选器**&#x200B;图标，然后选择以下所需的筛选器：

   ![](images/find-filters-source-mode.png){width="350"}

   - **文件类型**：选择要搜索文本的文件类型；**主题**，以及&#x200B;**映射**
   - **文档状态**：您可以从可用选项中选择文档状态。 显示的文档状态选项是从“文件夹”配置文件派生的。 它们表示这些用户档案中所有可能的文档状态的组合集。 默认状态包括“审核中”、“完成”、“草稿”、“已批准”、“编辑”和“已审核”。
   ...
   - **其他**：您可以配置以下内容：
     - **标记**：根据标记筛选内容。
     - **区分大小写的搜索**：启用搜索，确保搜索结果与指定的字母大小写完全匹配。
     - **列出其他人锁定的文件**：显示其他用户当前锁定的文件，在解除锁定之前禁止编辑。

1. 按Enter或选择&#x200B;**搜索**&#x200B;图标以执行搜索。

   ![](images/search-icon.png){width="350"}

1. 从搜索结果列表中选择文件。 文件将在Source视图中打开，搜索词将在内容中突出显示。

1. 在&#x200B;**替换为**&#x200B;字段中输入要用作替换的术语。 要自定义替换的应用方式，请单击字段旁边的&#x200B;**设置**&#x200B;图标，然后从可用选项中进行选择。

   - **替换已解锁的文件**：选择此选项以允许替换已解锁的文件。

   - **在替换后创建新版本**：如果要创建主题的新版本，并且选择替换文本，请选择此选项。 您还可以提供版本注释，这些注释将随每个更新的文件一起添加。 如果不选择此选项，则更改将保存在主题的当前版本中，并且不会创建新版本。

   ![](images/replace-settings-source-mode.png){width="350"}


1. 选择&#x200B;**替换匹配项**&#x200B;以替换主题中当前突出显示的搜索字符串，或者选择顶部和底部箭头以移动到文本的下一个或上一个匹配项。

   ![](images/replace-occurrence.png){width="350"}

1. 选择&#x200B;**全部替换**&#x200B;可一次性使用指定的替换字符串替换所有搜索文件中的搜索字符串的所有匹配项。 替换所有匹配项后，您将会看到通知。

   ![](images/replace-all.png){width="350"}

   >[!NOTE]
   >
   >要启用&#x200B;**全部替换**&#x200B;按钮，您的文件夹配置文件管理员或系统管理员必须在&#x200B;**Workspace设置**&#x200B;中的&#x200B;**常规**&#x200B;选项卡下选择选项&#x200B;**启用全部替换**（对于&#x200B;**内部部署，版本5.2**&#x200B;之前的安装程序，显示为&#x200B;**设置**）。

1. 您还可以将鼠标悬停在搜索结果列表中的文件上，查看其右侧的&#x200B;**替换文件中的所有项**&#x200B;图标，该图标允许您替换单个文件中出现的该术语的所有项。

   >[!NOTE]
   >
   > 您还获得&#x200B;**删除**&#x200B;图标以从搜索结果中删除该文件。 您删除的文件会移出列表，其中不会替换搜索项。

   ![](images/replace-all-in-file.png){width="350"}

1. “全部替换”操作完成后，将生成可下载的CSV报告，并提供执行的所有替换操作的快照。 您可以下载报告以查看有关替换操作的详细信息，包括成功替换的实例数、每个实例的详细信息，以及任何失败及其相应原因。 由于某些特定原因，该操作可能会失败，例如文件被其他用户锁定、操作期间对文件所做的更改导致的验证错误或其他类似问题。

   ![](images/snapshot-replace-all.png){width="350"}

在整个系统中，一次只能执行一项“全部替换”操作，并且在执行操作之前，您将看到“全部替换”状态。 您还可以中止介于两者之间的替换所有操作。 如果中止该操作，您将在收件箱中收到关于该操作的通知。
...
- **不使用Source模式：**&#x200B;搜索仅限于在“创作”视图中显示的文本，仅侧重于跳过源内容（如XML元素或属性）的可见内容。 此模式非常适用于仅包含内容的快速搜索。

  ![](images/map-find-replace-without-source-mode.png)

<br>
<details>
    <summary> 不使用Source模式执行全局搜索和替换</summary>


要在没有Source模式的情况下执行全局搜索和替换，请执行以下步骤：
...
1. 从搜索结果列表中选择文件。 文件将在内容编辑区域的“作者”视图中打开，搜索词将在内容中突出显示。

1. 在&#x200B;**替换为**&#x200B;字段中输入要用作替换的术语。 要自定义替换的应用方式，请单击字段旁边的&#x200B;**设置**&#x200B;图标，然后从可用选项中进行选择。

   - **在替换前锁定文件**：如果要在替换搜索字符串前自动锁定文件，请选择此选项。 如果您的管理员启用了配置以在编辑之前锁定文件，则此设置更相关。 启用后端设置后，您应该选择此选项。 它将阻止文件锁定对话框在做出任何更改之前提示您锁定每个文件。 如果不选择此选项，则在打开文件进行编辑之前将显示提示。
...
1. 选择&#x200B;**全部替换**&#x200B;可一次性使用替换项替换单个文件中搜索项的所有匹配项。 替换选定文件中的所有匹配项后，您将看到通知。

   >[!NOTE]
   >
   > 要启用“然后&#x200B;**全部替换**”图标，您的文件夹配置文件管理员或系统管理员必须在&#x200B;**Workspace设置**&#x200B;中的&#x200B;**常规**&#x200B;选项卡下选择选项&#x200B;**启用全部替换**（对于&#x200B;**内部部署，在版本5.2**&#x200B;之前安装程序，显示为&#x200B;**设置**）。 如果在替换操作过程中发生错误，将由于XML解析问题或DITA相关错误而跳过这些文件。

   ![](images/replace-all.png){width="350"}

1. 您还可以将鼠标悬停在搜索结果列表中的文件上，查看其右侧的&#x200B;**全部替换文件**&#x200B;图标，该图标允许您替换单个文件中出现的该术语的所有实例。
