...
Experience Manager Guides支持多种类型的引文，例如现代语言协会(MLA)、美国心理协会(APA)、芝加哥、电气和电子工程师协会(IEEE)、美国心脏协会(AHA)和美国医学协会(AMA)。 建议明确和一致地使用这些指标。


>[!NOTE]
>
>在发布到本机PDF时，Experience Manager Guides支持所有引文样式。 对于AEM Sites输出，当前仅支持AMA样式引文。
...
1. 填写&#x200B;**添加引文**&#x200B;对话框中的字段。
   ...
   >[!NOTE]
   >
   >您还可以提供ISBN、DOI或PubMed ID。 AEM Guides会自动填充其他字段。

   | 书籍 | 网站 | 日志 |
   | --- | ---|---|
   | **Source** <br>从下拉列表中，选择引用作为书籍的来源。 | **Source**<br>&#x200B;从下拉列表中，选择引用源作为网站。 | **Source** <br>从下拉列表中，选择引用源作为日志。 |
   | **搜索依据** <br>从下拉列表中选择&#x200B;**ISBN**&#x200B;或&#x200B;**DOI**&#x200B;以搜索链接到引文的数字ID。 <br> DOI：数字对象标识符<br> ISBN：唯一数字帐簿标识符 | **搜索依据** <br>从下拉列表中选择&#x200B;**DOI**&#x200B;以搜索链接到引文的数字ID。 | **搜索依据** <ul><li>从下拉列表中选择&#x200B;**DOI**&#x200B;或&#x200B;**PubMed ID**&#x200B;以搜索链接到引文的数字ID。</li><li>从下拉列表中选择&#x200B;**任何字段**&#x200B;以搜索所有日志字段，如&#x200B;*Title*、*Journal title*、*Author*、*Year*、*Volume*、*Number*&#x200B;和&#x200B;*Pages*。<br> 搜索会根据输入的文本返回最接近的匹配引用。</li><li> 从下拉列表中选择&#x200B;**分析引文**&#x200B;以分析支持的AMA引文并自动填充引文字段。 |
...
在主题中插入引文后，可在编辑器中预览它们。 您还可以使用本机PDF发布包含引文的内容。
