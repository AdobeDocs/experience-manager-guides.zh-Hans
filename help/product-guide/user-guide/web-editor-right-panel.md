...
对于引用的内容，面板还显示&#x200B;**链接路径**&#x200B;和&#x200B;**链接UUID**&#x200B;选项，这些选项可帮助您识别和复制选定的引用。

>[!NOTE]
>
> 对于基于HTML的文件，“链接路径”和“链接UUID”选项不可用。 这些文件继续使用现有的&#x200B;**链接URL**&#x200B;行为。

![](images/web-editor-cross-ref-options.png)

**类型**：查看并从下拉列表中为当前标记选择完整层次结构的标记。

**链接路径**：显示所选引用的相对路径。 使用&#x200B;**复制路径**&#x200B;来复制绝对路径。

**链接UUID**：显示所选引用的UUID。 使用&#x200B;**复制UUID**&#x200B;来复制UUID。

如果将有效的UUID直接粘贴到链接路径字段，则它会自动解析为绝对文件路径，并且相应的UUID会显示在链接UUID字段中。 这样可以更轻松地识别和复制资产路径及其基于UUID的引用。
...
>[!NOTE]
>
> “文件属性”面板提供用于查看和修改与文件关联的各种元数据属性的选项。 但是，当文件处于只读模式时，无法修改这些元数据属性。 此限制仅适用于DITA和Markdown文件。 对于非DITA资产（例如图像和多媒体），即使处于只读模式，元数据属性也保持可编辑状态。
...
除了打开文件之外，您还可以使用“引用”部分中的&#x200B;**选项**&#x200B;菜单执行许多操作。 您可以执行的某些操作包括编辑、预览、复制UUID、复制路径、添加到收藏集和属性。

**翻译**

此部分按字母顺序列出编辑器中当前打开的资产的所有可用语言副本。 该信息以表格视图显示，显示每个语言代码以及相应的&#x200B;*文件标题*（或者&#x200B;*文件名*，如果&#x200B;*文件标题*&#x200B;不可用）。
