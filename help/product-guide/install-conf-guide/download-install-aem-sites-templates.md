...
以下选项卡根据您的Experience Manager Guides设置提供了必要的先决条件：Cloud Service或内部部署。
...
以下选项卡提供了基于您的Experience Manager Guides设置的软件包安装说明：Cloud Service或内部部署。
...
2. **配置AEM站点预设：**&#x200B;有两个选项可用于配置现成(OOTB)站点：
   ...
   **选项2：使用站点路径**
   ...
   1. 为Cloud Service手动将&#x200B;**站点路径**&#x200B;设置为`/content/AEMG-Docs-Site/en/docs/product`，为内部部署手动设置`/content/aemg-docs/en/docs/product1`。
...
1. **生成站点：**
   ...
   2. 生成的站点在以下路径中可用： `/content/AEMG-Docs-Site/en/docs/product`适用于Cloud Service，`/content/aemg-docs/en/docs/product1`适用于内部部署。
