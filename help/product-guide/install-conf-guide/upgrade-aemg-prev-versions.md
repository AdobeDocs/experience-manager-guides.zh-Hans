...
如果您使用的是3.8.5 **之前的**&#x200B;版本，请参阅[Experience Manager Guides帮助PDF存档](https://helpx.adobe.com/cn/xml-documentation-for-experience-manager/archive.html)上提供的产品特定安装指南中的&#x200B;**升级Adobe Experience Manager Guides**&#x200B;部分。
...
>[!NOTE]
>
> 此升级过程仅适用于&#x200B;**从** 3.8.5 **到** 4.0 **的**。 有关从&#x200B;**3.4或更高版本**&#x200B;升级到&#x200B;**3.8.5**&#x200B;的信息，请参阅[Adobe Experience Manager Guides帮助PDF存档](https://helpx.adobe.com/cn/xml-documentation-for-experience-manager/archive.html)上提供的产品特定安装指南。
...
2. 已关闭所有翻译任务。
3. 将`com.adobe.fmdita.translationservices.TranslationMapUpgradeScript`的日志级别设置为`INFO`并登录到新日志文件（例如，`logs/translation_upgrade.log`）。
...
1. 从[Adobe软件分发门户](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)下载&#x200B;**4.2**&#x200B;包。
2. 安装4.2包。
3. 安装后，请等待日志中显示以下消息：
...
2. 关闭所有翻译任务。
3. 将`com.adobe.fmdita.translationservices.TranslationMapUpgradeScript`的日志级别设置为`INFO`并登录到新文件（例如，`logs/translation_upgrade.log`）。
...
1. 从[Adobe软件分发门户](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)下载&#x200B;**4.2.1**&#x200B;包。
2. 安装4.2.1包。
...
如果您使用的是版本&#x200B;**4.3.1**，则可以直接升级到&#x200B;**4.3.1.5**。

### 安装版本4.3.1.5

1. 从[Adobe软件分发门户](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)下载&#x200B;**4.3.1.5**&#x200B;包。
2. 安装4.3.1.5包。
3. 等待安装过程成功完成。
4. 继续升级自定义项，如下一节所述。
...
如果您正在使用&#x200B;**4.3.1**、**4.3.0**、**4.2**&#x200B;或&#x200B;**4.2.1 （修补程序4.2.1.3）**，则可以直接升级到&#x200B;**4.4.0**。
...
1. 在`damAssetLucene` oak索引中，在`/oak:index/damAssetLucene/indexRules/dam:Asset`中添加布尔属性`indexNodeName=true`。
2. 在`/oak:index/damAssetLucene/indexRules/dam:Asset/properties`下添加名为`excerpt`的新节点，并设置源中显示的属性：
   - `name=rep:excerpt`
   - `propertyIndex=true`
   - `notNullCheckEnabled=true`
3. 通过设置`reindex=true`重新索引`damAssetLucene`并等待它再次变为`false`（可能需要几个小时）。
...
1. （可选）增加适用于大型存储库的Oak queryLimitReads
   ...
   如果存在超过&#x200B;**100,000个DITA文件**，请将`org.apache.jackrabbit.oak.query.QueryEngineSettingsService`下的`queryLimitReads`更新为大于资产数的值（例如： `200000`），重新部署，然后继续。
...
- 使用大于50 **的`order`值**，因为指南使用的是`order=50`。
