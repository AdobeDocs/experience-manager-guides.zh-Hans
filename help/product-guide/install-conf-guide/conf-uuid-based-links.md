...
默认情况下，在编辑器中使用“插入引用”或“插入重用内容”选项创建链接时，将使用引用内容的UUID创建链接。 引用内容的&#x200B;**Link**&#x200B;属性\（在“属性”面板中\）可以配置为显示引用内容的相对文件路径或UUID。 对于Cloud Service，默认情况下，引用内容的UUID会显示在“属性”面板中。 对于内部部署，此显示由`configMgr`中的&#x200B;**启用UUID**&#x200B;选项控制。 默认情况下，此功能处于打开状态，这意味着引用内容的UUID显示在“属性”面板中。
