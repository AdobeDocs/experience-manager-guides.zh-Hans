...
以下选项卡提供了根据您的Experience Manager Guides设置启用&#x200B;**为上传的文件创建新版本**&#x200B;选项的说明： Cloud Service或内部部署。
...
以下选项卡提供了有关如何根据您的Experience Manager Guides设置配置设置以允许编辑已签出文件的说明：Cloud Service或内部部署。

>[!BEGINTABS]

>[!TAB Cloud Service]

使用[配置覆盖](download-install-config-override.md#)中提供的说明创建配置文件。 在配置文件中，提供以下\(property\)详细信息以配置对签出文件的编辑：
...
以下选项卡提供了相关说明，允许用户在上传时覆盖已由其或某些其他用户根据您的Experience Manager Guides设置签出的文件： Cloud Service或内部部署。
...
| PID | 属性键 | 属性值 |
|---|------------|--------------|
| `com.adobe.fmdita.config.ConfigManager` | `overwrite.checkout.onupload` | 布尔值\(true/false\)。<br> **默认值**： `false` |
...
以下选项卡提供了相关说明，可防止用户意外删除已由他们或其他一些用户根据您的Experience Manager Guides设置签出的文件： Cloud Service或内部部署。

>[!BEGINTABS]

>[!TAB Cloud Service]

使用[配置覆盖](download-install-config-override.md#)中提供的说明创建配置文件。 在配置文件中，提供以下\(property\)详细信息，以防用户意外删除已签出的文件：
...
以下选项卡提供了相关说明，用于定义谁可以删除包含引用或被基于您的Experience Manager Guides设置的其他文件引用的文件： Cloud Service或内部部署。

>[!BEGINTABS]

>[!TAB Cloud Service]

使用[配置覆盖](download-install-config-override.md#)中提供的说明创建配置文件。 在配置文件中，提供以下\(property\)详细信息以定义谁可以删除包含引用或被其他文件引用的文件：
...
在更新内容和创建新版本时，存储库中会维护文件的先前版本。 可能会在一段时间内为文件创建多个版本，这些版本可能会共同占用存储库中的大量空间。 AEM Guides允许您配置应从存储库中删除的旧版本。

>[!NOTE]
>
> 您可以清除DITA和非DITA文件。

符合任何给定标准的文件版本将得到维护且不清除：
...
1. 您可以选择&#x200B;**取消清除**&#x200B;或&#x200B;**开始清除**。
   ...
   选择&#x200B;**下载版本清除报告**&#x200B;以查看清除的版本。 此报表提供所有版本的清除状态，以及保留特定版本或清除该版本的原因。
