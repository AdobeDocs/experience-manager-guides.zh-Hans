...
### 将现有Word文档转换为DITA主题类型文档

以下选项卡提供了有关如何根据Experience Manager Guides设置将现有Word文档转换为DITA主题类型文档的说明：Cloud Service或内部部署。
...
1. 将源Word文档上传到此文件夹中。 有关在DAM上上传内容的信息，请参阅[上传现有DITA内容](upload-dita-content.md#)。
...
如果您是AEM Guides as a Cloud Service的现有用户，并且正在从2021年8月版本升级到2022年1月或更高版本，请更新给定属性，因为只有少数文件已移动。
...
概括地说，转换过程要求您[准备InDesign文件进行转换](aemg-appendix.md#id195DBF0045Z)和[准备映射文件以进行InDesign到DITA的迁移](aemg-appendix.md#id194AF0003HT)，然后您需要按照给定的过程运行转换过程。
...
1. 在`apps`节点内创建`config`文件夹的覆盖节点。
   ...
   - 在`inputDir`元素中，指定可在其中使用源InDesign文档的输入文件夹的位置。 例如，如果您的InDesign文档存储在`projects`文件夹中名为`indesigntodita`的文件夹中，则将位置指定为： `/content/dam/idmlfiles/indesigntodita/`
   ...
   - 在`mapStyle`元素中，指定映射文件的位置，该文件包含InDesign文档样式到DITA元素的映射。 默认映射存储在位于以下位置的文件中：
     ...
     >[!NOTE]
     >
     > 有关`stmap.adobeidml.xml`文件结构以及如何对其进行自定义的更多信息，请参阅[附录](aemg-appendix.md#id194AF0003HT)。
...
1. 将源InDesign文档上传到此文件夹中。 有关在DAM上上传内容的信息，请参阅[上传现有DITA内容](upload-dita-content.md#)。
...
以下选项卡提供了有关如何根据Experience Manager Guides设置将现有XHTML文档转换为DITA主题的说明：Cloud Service或内部部署。
...
1. 将源XHTML文档上传到此文件夹中。 有关在DAM上上传内容的信息，请参阅[上传现有DITA内容](upload-dita-content.md#)。
...
要将非结构化Adobe FrameMaker内容(.fm和.book)转换为结构化DITA，您可以使用FrameMaker的转换表机制。 此过程侧重于使用基于模板的方法评估现有内容，以及通过转化表将FrameMaker样式映射到DITA。 有关更多详细信息，请查看[在Adobe FrameMaker中将技术文档从非结构化迁移到DITA](https://migrate-from-unstructured-to-dita-step-by-step-guide.meetus.adobeevents.com/)。

转换后，结构化内容可迁移到AEM Guides中。 有关更多详细信息，请查看[使用WebDAV工具和FrameMaker上载现有DITA内容](./upload-dita-content.md)。

## 迁移任何其他结构化文档 {#id1949B0590YK}

AEM Guides允许您将现有结构化文档转换为有效的DITA文档。 您需要指定输入和输出文件夹位置、转换文件的位置、保存最终输出的扩展名以及是否需要文档的新版本。

以下选项卡提供了根据Experience Manager Guides设置迁移任何其他结构化文档的说明：Cloud Service或内部部署。
...
1. 将源结构化文档上传到此文件夹中。 有关在DAM上上传内容的信息，请参阅[上传现有DITA内容](upload-dita-content.md#)。
