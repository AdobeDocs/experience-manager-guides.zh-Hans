---
title: 生成SCORM输出
description: 了解如何在产品培训和学习中生成SCORM输出
feature: Authoring
role: User
exl-id: 3de623d6-c547-42a5-9ce4-c3e09accb8b1
TQID: https://experienceleague.adobe.com/bSlHWEHJz3axXD3x4mPioaYiId535wHu5m9a5L0jhdc
product_v2: id: fae5e35a-80c9-4b94-9352-1a060a6aab1did: fd1f54a9-f50c-467d-8956-cebbaf4f3eb8
feature_v2: id: ab01a588-7dea-43f2-a699-0b3f128465d6
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554
source-git-commit: 6c69cc5da7eadbcbfe499ca7b54c2ddaa1064db7
workflow-type: tm+mt
source-wordcount: 159
ht-degree: 0%

---

# 生成SCORM输出

执行以下步骤以生成SCORM输出：

1. 根据您的首选项为SCORM输出配置所有必需的设置后，导航到SCORM预设页面的工具栏。
1. 选择&#x200B;**生成输出**。

   ![](assets/scorm-generate-output-new.png){width="650"}

1. 生成完成后，将显示一条成功消息，确认已创建&#x200B;**filename.zip**&#x200B;文件。 您可以使用成功消息上的&#x200B;**查看输出**&#x200B;预览输出。

   ![](assets/scorm-success-message.png){width="350"}

1. 您可以通过分别选择&#x200B;**下载输出**&#x200B;或&#x200B;**预览输出**&#x200B;来下载或预览输出。

   >[!NOTE]
   >
   > 在生成SCORM输出时，可以使用SCORM预览。 如果您的课程依赖外部域提供字体、样式表或嵌入内容，请在[配置SCORM预览](../lc-config-guide/conf-scorm-preview-filters.md)中配置允许的域。

   ![](assets/scorm-view-output-new.png){width="650"}

您可以使用SCORM预设的&#x200B;**发布到LMS**&#x200B;选项卡上的&#x200B;**上传**&#x200B;选项将ZIP文件上传到LMS，以使学习者能够使用该课程。

