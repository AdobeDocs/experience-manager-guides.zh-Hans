...
| **嵌入多媒体文件** | 选择此选项以直接在生成的PDF中嵌入受支持的多媒体资源，例如音频和视频。 启用后，视频内容将在PDF中使用海报图像或缩略图进行表示，并且可以在支持嵌入多媒体的兼容PDF查看器中播放。 对于视频播放，请在支持的应用程序（如最新版本的Adobe Acrobat）中打开PDF，并在出现提示时信任该文档。 大多数基于浏览器的PDF查看器不支持嵌入式多媒体播放。 |
...
| **显示水印** | 选择此选项可在输出中叠加水印。 您可以在文本框中输入新的文本字符串，其字符大小写符合您的要求。 <br><br>使用静态文本或语言变量发布水印的本地化版本。  根据您选择的语言，将在PDF输出中自动选取本地化的值。 例如，您可以将“Publisher”作为水印以英语打印，将“Auteure”作为水印以法语打印。 <br> 格式： `${lng:<variable name>}`。 例如，`$ {lng:publisher-label}`其中`publisher-label`是语言变量。 |
