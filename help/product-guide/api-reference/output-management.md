...
|`templatePath`|\(对于AEM Site output\)用于生成所需输出的模板节点的路径。|
|`searchScope`|指定搜索操作的范围。 此参数的值必须设置为`local`。|
|`generateTOC`|\(用于AEM站点输出\)指定是否生成目录\(true\) \(false\)。|
|`generateBreadcrumbs`|\(用于AEM站点输出\)指定是否生成痕迹导航\(true\)\(false\)。|
|`overwriteFiles`|\(用于AEM站点输出\)指定是否覆盖目标上的文件\(true\) \(false\)。|
|`pdfGenerator`|指定要使用的PDF生成引擎。 可能的值为： <br>-   DITAOT <br>-   FMPS|
...
|`outputName`|字符串|是|要用于生成输出的输出预设的名称\(s\)。 可以使用管道分隔符(“\|”\)指定多个输出预设，例如`aemsite|pdfoutput`。|
...
|`contentPath`|JSON|是|DITA映射文件和主题文件的绝对路径以及输出预设的名称。 使用以下示例作为构建块：|
