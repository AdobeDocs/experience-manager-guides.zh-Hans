...
1. 将云服务Git代码的`/dox/dox.installer/pom.xml`文件中的`<dox.version>`属性更新为2022.9.178。
...
* 原生PDF | `xref`对于特定DITA主题未正确解析概念。 (10229)
* 原生PDF |无法在生成的PDF输出中查看表的标题文本。 (9827)
* 原生PDF |附录中的引用不会在PDF输出中显示为附录。 (10182)
* 原生PDF |表的Frame属性不会传播到temp HTML（作为类）。 (10353)
* 原生PDF | temp HTML文件将colsep和rowsep类添加到td和th，即使它们在源DITA中的值为0。 (10352)
* 原生PDF |不处理在页面布局中添加的标准的元数据。 (10377)
* 原生PDF |针对特定内容生成PDF失败。 (9927)
* 原生PDF |通过conkeyref的内容未显示在PDF输出中。 (9836)
