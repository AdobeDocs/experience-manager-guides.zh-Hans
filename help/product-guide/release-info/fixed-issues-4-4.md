...
- 即使用户未从&#x200B;**用户首选项**&#x200B;中明确设置rootmap设置，该设置仍会保留在编辑器中。 (11551)
- 应用了某些属性的内容在&#x200B;**作者**&#x200B;或&#x200B;**预览**&#x200B;模式下未突出显示。 (11063)
- 在编辑器中，插入`varname`标记时会打开&#x200B;**键引用**&#x200B;窗口。 (10940)
- 将词汇表主题从存储库拖到词汇表映射中会创建`topicref`。 (10767)
- 在Google Chrome和Microsoft Edge浏览器中，XML编辑器的“预览”窗口被截断。 (10755)
- 编辑器无法将元素包装在可能的父元素中。 (8791)
- 重新安装Adobe Experience Manager Guides版本4.3.1后，将卸载该编辑器。 (14519)
...
- **AEM Site**&#x200B;预设不允许在编辑器中选择`/content`层次结构之外的模板。 (14260)
...
- 在AEM站点发布中，将带有范围对等链接的大型DITA映射提交到数据存储时发生问题。 (13530)
- 本机PDF发布中使用的模板的&#x200B;**页面布局**&#x200B;工具栏中的&#x200B;**编辑内容**&#x200B;选项显示的图标和工具提示不正确。 (13492)
- 无法使用Experience Manager Guides **批量发布仪表板**&#x200B;激活站点。 (13439)
...
- 通过编辑器UI创建的文件夹配置文件级别输出预设中缺少&#x200B;**ditaval**&#x200B;选项。 (11903)
- 在&#x200B;**AEM Site**&#x200B;预设中，**为每个主题**&#x200B;生成单独的PDF的选项不起作用。 (11555)
...
- **基线筛选器**&#x200B;文件未在编辑器中使用文件名。 (13486)
- 在编辑器中，基线显示DITA文件的先前版本（而非所选版本）的标题。 (13444)
- 禁用父DITA映射的索引以获取更好的性能可能会影响某些功能的功能。(12213)
- 未创建大型DITA映射的条件预设。 (10936)
- 编辑映射收藏集时，无法编辑收藏集第一个映射的预设。 (10649)
- `labels.json`文件中的标签在编辑器中以随机顺序显示。 (10508)
...
- 在编辑器的上一个版本和当前版本的并排审核面板中会发生对称问题。 (14156)
