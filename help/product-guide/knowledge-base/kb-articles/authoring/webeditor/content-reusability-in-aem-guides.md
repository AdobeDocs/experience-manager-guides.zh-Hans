...
Adobe AEM Guides利用DITA的优势为内容重用提供用户友好的界面。

本文将讨论：

1. [使用主题引用(`topicrefs`)可重复使用](#reusability-using-topic-referencestopicref)
2. [使用内容引用（`conref`和`conkeyref`）可重复使用](#reusability-using-content-reference-conref--conkeyref)
