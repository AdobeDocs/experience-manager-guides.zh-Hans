...
设计页面布局时，您可以定义构成页面的各种元素。 例如，可以在页面上定义页面大小、边距、页眉和页脚、方向以及其他页面规范。 本机PDF发布功能允许您根据[页面媒体标准](https://www.w3.org/TR/css-page-3/)设计页面。 使用本机PDF发布功能的用户界面，可以轻松自定义分页介质标准中涵盖的大多数设置。 对于其他高级级格式，您可以使用Source视图编写您自己的CSS代码。

设计页面布局后，需要将这些布局与“PDF页面布局”设置中各自的节相关联。 有关如何创建和打开要进行自定义的页面布局的详细信息，请参阅[创建和自定义页面布局](components-pdf-template.md#create-customize-page-layout)部分。
...
通过&#x200B;**模板>设置**&#x200B;部分下的“页面布局”设置，您可以定义用于PDF中不同部分的页面布局。 每个页面布局还可以有不同的第一页、右页或左页变体。
...
除了预定义字段外，您还可以将元数据信息作为变量或字段添加到页面布局中。 此元数据存储在源DITA **映射内容**&#x200B;中，或者可以从DITA **映射文件属性**&#x200B;或&#x200B;**主题文件属性**&#x200B;中选取它并轻松地插入到页面布局中。
...
可以在单个文档中合并&#x200B;**映射文件属性**&#x200B;和&#x200B;**主题文件属性**&#x200B;中的元数据。 例如，您可以发布一个PDF，其封面页具有地图标题，而其他页面标题具有主题标题。 为此，您可以将&#x200B;**映射文件属性**&#x200B;中的映射标题元数据添加到封面布局。 然后，将主题标题元数据从&#x200B;**主题文件属性**&#x200B;添加到章节和主题页面布局的标题中。
