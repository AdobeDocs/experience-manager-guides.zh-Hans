...
默认情况下，在DITA主题中添加的草稿注释会从本机PDF输出中排除。 这是由输出模板的内容样式表中的`draft-comment`样式控制的，其中`display`属性设置为`none`。

作为管理员，您可以更新此样式，以便在输出中显示草稿注释：

1. 在&#x200B;**输出模板**&#x200B;面板中，打开映射用于PDF发布的模板。
2. 展开&#x200B;**样式表**&#x200B;并双击&#x200B;**内容**&#x200B;以打开内容样式表。
3. 在&#x200B;**样式**&#x200B;面板中，找到并选择&#x200B;**草稿注释**。

   如果列表较长，请使用搜索字段快速查找。

4. 在&#x200B;**属性**&#x200B;面板中，将&#x200B;**显示**&#x200B;属性的值从`none`更改为可见值（如`block`、`inline-block`、`grid`等）。

   ![在内容样式表中配置草稿注释属性](./assets/draft-comment-setting.png)

5. 保存样式表。

>[!NOTE]
>
>此样式控制草稿注释通常在样式化内容中是否可见。 若要在&#x200B;**本机PDF**&#x200B;输出中包含草稿注释，您还必须在Native PDF输出预设中启用&#x200B;**包含草稿注释**&#x200B;选项。 有关详细信息，请查看[Native PDF输出预设](../web-editor/native-pdf-web-editor.md)。 草稿注释需要同时使用这两项设置，才能在生成的PDF中完全按照编辑器中显示的形式显示草稿注释。
...
您可以定义要在断点前后显示的文本。 例如，第5页上有表分隔，而您为&#x200B;**分隔前文本**&#x200B;定义`<Continued on page %page-num%>`。  文本在第5页的底部显示“第6页继续”。
  ...
  >[!NOTE]
  >
  > 在&#x200B;**断点前文本**&#x200B;或&#x200B;**断点后文本**&#x200B;字段中使用语言变量时，请确保在单个语言变量中定义整个文本。 在这些字段中的语言变量之外添加的任何文本或变量都不会呈现。 例如，不要在&#x200B;**Text After Break字段**&#x200B;中使用`${lng:Continued-from-page} %page-num%`等变量的组合，而只在该字段中使用`${lng:Continued-from-page}`，并将`Continued-from-page`语言变量的值单独设置为`Continued-from-page %page-num%`。
...
* **空页面的页面布局**：您还可以为空页面指定页面布局。 所选布局将应用于所有空页面。 例如，如果您为所有空白页面设计了空白页面布局，则在下拉列表中选择&#x200B;**空白**，文档中的所有空白页面都将具有空白页面布局。
...
>[!NOTE]
>
> 此设置仅适用于部分，而不适用于其组件。  例如，如果为&#x200B;**章节和主题**&#x200B;选择&#x200B;**上一页**&#x200B;选项，则&#x200B;**章节和主题**&#x200B;部分将与上一页合并。 根据&#x200B;**常规**&#x200B;设置发布各种章节和主题。例如，如果在&#x200B;**Start any new chapter from setting**&#x200B;中，您选择了&#x200B;**Odd Page**，则在结束于奇数页的章节之后插入空白页。
