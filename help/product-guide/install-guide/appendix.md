---
title: 附录
description: 了解如何准备InDesign文件进行转换
...
InDesign为作者提供了一组丰富的功能，用于创建吸引人和复杂的文档。 通常，这意味着文档的各个部分会以可视方式放置在页面上，但不尝试在这些文本框架之间提供任何流向。 未定义文本框架的“*读取顺序*”时，IDML文件将包含可能未遵循任何有意义的顺序的内文。 最终结果将是一个或多个DITA主题，其中包含以随机顺序排列的段落、表格和图形。

虽然可以在DITA编辑器中将DITA内容编辑为合理的顺序，但在创建IDML文件之前修复InDesign文件会容易得多。 可以在不改变源文档外观的情况下完成此操作。 通过正确定义阅读顺序，它还有助于使源文档可访问。
...
如果需要将某些&#x200B;*标题1*&#x200B;段落转换为其他DITA文档类型，请在InDesign中复制段落样式。 根据需要，为这些样式提供一个易于识别的名称，例如&#x200B;*Heading1\_genTask*&#x200B;或&#x200B;*Heading1\_troubleshooting*。 然后，设置mapStyle文件，如下所示：
...
InDesign与XML的关系并不密切。 虽然文档可以包含XML DTD，并且主故事可能对该DTD有效，但也可以创建一些内容是XML但不包含DTD的混合文档。 这些是成功转换为DITA的不希望出现的情况。 如果文档包含XML部件，请尝试将输出保存为XML，并查看结果是否可接受。 如果不包含，则DITA内容也将包含无效内容，或者可能完全失败。

***表格格式***

从InDesign表格格式规则转换为DITA中的等效表格格式是一个复杂的过程。 这是因为与DITA中使用的Oasis \(CALS\)表模型提供的基本选项相比，源文件中提供了丰富的格式功能。 提供了垂直和水平文本对齐方式，并且提供了类似的结果，尽管两端对齐的文本始终根据文本方向对齐，而InDesign允许“两端对齐”和“两端对齐”。
...
## 为InDesign到DITA的迁移准备映射文件 {#id194AF0003HT}
...
在上述示例中，`paraRule` = &quot;Heading1&quot;包含两个`@style`元素。 只需创建一个等效的`doctypeParaRule`元素，并根据需要设置`@mapToDoctype`属性。
...
- `@wrapper`：包装元素的名称。
- `@context`：提供进一步细化给定元素包装方式的方法。 以下示例显示了一种根据`li`值\（上下文定义在`ol`元素\）映射有序列表`ul`或无序列表`@context`中的一系列`paraRule`元素的方法：
...
以下示例显示如何从`fig`和`title`元素创建`image`元素：

- `@elements`：用逗号分隔的列出元素将封装在`@wrapper`属性中命名的元素中。 由于在图像下方包含图形标题的常见做法，标题将为`title`之后紧接的`image`元素。
...
- `@elements`：位于`title`之前或之后的`table`元素将封装在`@wrapper`属性中命名的元素中。 XPath样式谓词可以将title元素的位置标识为`[before]`或`[after]`。
...
`paraRule`元素是必需的。 这会指定所有段落样式的映射规则。 在InDesign文档中，所有文本都包含在段落样式的子结构中，即使没有任何样式的段落也被命名为`[No paragraph style]`。 方括号，表示内置的InDesign样式名称。
...
>[!NOTE]
>
> 在`[No character style]`时，内置字符样式`local="0"`将没有映射，因为它们在预处理期间被删除。
...
**`elementRule`元素** `elementRule`元素是必需的。 这些是所有源元素的映射规则。 虽然InDesign文档包含非结构化样式元素，但除非启用“***混合模式***”处理，否则结构化内容将忽略这些元素。

`elementRule`中使用的属性说明如下：

- `@elementName`：源InDesign文档中的元素的名称。
...
1. &#x200B;
   - 检查`references_backup_<timestamp>"`下是否创建了新节点“`/content/fmdita`”
- 检查引用是否已修复
