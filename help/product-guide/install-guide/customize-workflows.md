...
| `payloadJson` | JSON 对象 | 指定以下值： <br> - `base`：包含已发送审核主题的父文件夹的路径。<br>- `asset`：已发送审核主题路径。 <br>- `referrer`：将其留空。 |
...
AEM Guides包含一组电子邮件和AEM通知，您可以对其进行自定义。 执行以下步骤以自定义这些通知：
...
>[!NOTE]
>
> 要自定义或扩展默认的后处理工作流，您可以使用&#x200B;*API参考*&#x200B;中所述的后处理事件处理程序。
